...
#define __pi_c 3.141592653589793238462643f
#define __ln10 2.302585092994046f

static REALTYPE__ factorials[] = {1.0f, 1.0f, 2.0f, 6.0f, 24.0f, 120.0f, 720.0f, 5040.0f, 40320.0f, 362880.0f, 3628800.0f, 39916800.0f, 479001600.0f, 6227020800.0f, 87178291200.0f,
...
static REALTYPE__ CnkS_123[] = {0.0f, 1.3333333e-01f, 7.6190476e-02f, 5.0793651e-02f, 3.6940837e-02f, 2.8416028e-02f, 2.2732823e-02f, 1.8721148e-02f,
...
static REALTYPE__ CnkS_121[] = {0.0, 6.6666667e-01f, 5.3333333e-01f, 4.5714286e-01f, 4.0634921e-01f, 3.6940837e-01f, 3.4099234e-01f, 3.1825952e-01f,
...
#define __3 3.0f
#define __4 4.0f
#define __onehalf 1.5f
#define __025 0.25f
#define __05 0.5f
#define __15to8 (15.0f/8.0f)
#define __safe 1.00001f

#define __220 220.0f
#define __10000 10000.0f
#else
#define REALTYPE_MAX DBL_MAX
#define __pi_c 3.141592653589793238462643
#define __ln10 2.302585092994046

static REALTYPE__ factorials[] = {1.0, 1.0, 2.0, 6.0, 24.0, 120.0, 720.0, 5040.0, 40320.0, 362880.0, 3628800.0, 39916800.0, 479001600.0, 6227020800.0, 87178291200.0,
...
static REALTYPE__ CnkS_123[] = {0.0, 1.3333333e-01, 7.6190476e-02, 5.0793651e-02, 3.6940837e-02, 2.8416028e-02, 2.2732823e-02, 1.8721148e-02,
...
static REALTYPE__ CnkS_121[] = {0.0, 6.6666667e-01, 5.3333333e-01, 4.5714286e-01, 4.0634921e-01, 3.6940837e-01, 3.4099234e-01, 3.1825952e-01,
...
#define __3 3.0
#define __4 4.0
#define __onehalf 1.5
#define __025 0.25
#define __05 0.5
#define __15to8 (15.0/8.0)
#define __safe 1.00001
#define __gammaonehalf 1.772453850905516027298167
#define __sqrtpi __gammaonehalf

#define __220 220.0
#define __10000 10000.0
...
inline REALTYPE__ Rad2Deg(REALTYPE__ x)
{
    return x*__180/__pi_c;
}

inline REALTYPE__ Deg2Rad(REALTYPE__ x)
{
    return x*__pi_c/__180;
}

// __erfc = 1-__erf
//   __erf(x)  = 2/sqrt(pi) * Integral(exp(-t^2) dt) [0...x]
//   __erfc(x) = 2/sqrt(pi) * Integral(exp(-t^2) dt) [x..+inf]
//   NB: __erf(0)  = 0 __erf(+inf)  = 1
//   NB: __erfc(0) = 1 __erfc(+inf) = 0
inline REALTYPE__ __erfc(REALTYPE__ _x)
{
// Abramowitz, Stegun, pp.120-122 (russ. transl.)
// abs.error <= 3e-7

    REALTYPE__ x = fabs(_x);
#ifdef AS_REAL_AS_FLOAT
	REALTYPE__ v = (((((0.0000430638f*x + 0.0002765672f)*x + 0.0001520143f)*x + 0.0092705272f)*x
				+ 0.0422820123f)*x + 0.0705230784f)*x + 1.0f;
#else
	REALTYPE__ v = (((((0.0000430638*x + 0.0002765672)*x + 0.0001520143)*x + 0.0092705272)*x
	...
	REALTYPE__ res = __1/v;

    return (_x < 0 ? 2 - res : res);
}

inline REALTYPE__ __gamma(REALTYPE__ _x) // 1 <= x <= 33
{
    _x -= 1;
...
inline double __getmax(REALTYPE__ x1, REALTYPE__ x2)
{
    return x1 > x2 ? x1 : x2;
}

inline double __getmin(REALTYPE__ x1, REALTYPE__ x2)
{
    return x1 < x2 ? x1 : x2;
}

inline double __dmax(int n, REALTYPE__ *x, int *_im = nullptr)
{
    double m = -DBL_MAX;
    int im = -1;
    for (int i = 0; i < n; i++)
...
inline double __dmin(int n, REALTYPE__ *x, int *_im = nullptr)
{
    double m = -DBL_MAX;
    int im = -1;
    for (int i = 0; i < n; i++)
...
inline bool __interpInArray(int n, REALTYPE__ *x, REALTYPE__ *Y, REALTYPE__ x0, REALTYPE__ *Y0)
...
inline bool __interpInArrayEqdist(int n, REALTYPE__ *x, REALTYPE__ x0, REALTYPE__ *Y0)
{   // suppose x is sorted ascending
    if (x0 < x[0])
        return false;
    if (x0 > x[n - 1])
